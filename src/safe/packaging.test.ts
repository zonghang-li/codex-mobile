...
describe('local installation packaging', () => {
  it('defines clone-local install and service lifecycle scripts', async () => {
    ...
    expect(packageJson.scripts?.['network:status']).toBe('sh scripts/codex-mobile-network-mode.sh status')
    expect(packageJson.bin?.['codex-mobile']).toBe('dist-cli/index.js')
    expect(packageJson.bin?.['codex-mobile-safe']).toBe('dist-cli/safe.js')
    expect(packageJson.bin?.['codex-mobile-network']).toBe('scripts/codex-mobile-network-mode.sh')
    expect(packageJson.files).toContain('src/')
    expect(packageJson.files).toContain('scripts/codex-mobile-network-mode.sh')
  })

  it('packages a network mode helper that separates LAN and Tailnet exposure', async () => {
    const helperPath = fileURLToPath(new URL('../../scripts/codex-mobile-network-mode.sh', import.meta.url))
    const helper = await readFile(helperPath, 'utf8')
    await execFileAsync('sh', ['-n', helperPath])

    expect(helper).toContain('approval_policy=${CODEX_MOBILE_APPROVAL_POLICY:-never}')
    expect(helper).toContain('while [ -L "$script_path" ]')
    expect(helper).toContain('sandbox_mode=${CODEX_MOBILE_SANDBOX_MODE:-workspace-write}')
    expect(helper).toContain('tailscale down')
    expect(helper).toContain('tailscale up')
    expect(helper).toContain('tailscale serve reset')
    expect(helper).toContain('codex-mobile-safe-lan')
    expect(helper).toContain('codex-mobile-safe-tailnet')
    expect(helper).toContain('--lan')
    expect(helper).toContain('expose tailscale')
    expect(helper).toContain('--password-file "$password_file"')
    expect(helper).not.toMatch(/--password(?:=|\s)/u)
    expect(helper).not.toContain('funnel')
    expect(helper).not.toContain('cloudflared')
  })
