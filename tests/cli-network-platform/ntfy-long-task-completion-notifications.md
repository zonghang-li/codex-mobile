...
- Linux with Node.js 22.13+, pnpm, systemd user services, and Tailscale installed and logged into the intended tailnet.
...
15. Confirm only the exact top-level sources `cli`, `vscode`, `exec`, and `appServer` qualify. A non-empty parent-thread alias or a `subagent`/`subAgent` source must suppress lifecycle delivery, and unknown, malformed, or unreadable hierarchy must fail closed. Confirm the notifier repeats this check through one `thread/read` after the duration gate, reuses that verified read for the matching turn's final assistant summary, and silently clears suppressed legacy active state without creating pending work.
...
8. In the same harness, complete a turn at exactly `600_000` ms and observe exactly one captured ntfy send.
9. In the deterministic integration harness, feed one logical turn through both direct app-server notification handling and external rollout discovery. Observe exactly one captured send, one durable turn key, one stable sequence ID, and no duplicate pending or sent record.
10. Start another controlled external turn, restart the safe service between its authoritative rollout start and terminal records, then complete it. Observe exactly one send using the original rollout timestamp rather than the restart time.
11. Complete a historical rollout fixture before monitor startup, start the monitor, and observe no send. Also remove its writer/process evidence without a terminal lifecycle record and confirm no completion is inferred.
12. In the deterministic fake-clock harness, keep a missing-writer active cursor below and then at the 24-hour inactive boundary. Confirm append checks continue before the boundary, eviction at the boundary emits nothing, and released cursor capacity can admit a newly discovered writer.
