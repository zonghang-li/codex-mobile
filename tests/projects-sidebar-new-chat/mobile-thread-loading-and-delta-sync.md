...
3. Confirm the first sidebar page requests and returns only the 5 most recent threads even when imported state-database threads exist, preserves the native cursor, contains no `turns`, `items`, `messages`, `transcript`, or `externalRuntime`, keeps the selected thread usable if that list is still refreshing, and loads older sidebar rows asynchronously.
...
25. With DevTools recording from before navigation, open and hard-refresh that CLI-owned thread without interacting with the composer; confirm the browser issues no `thread/resume`, `turn/start`, `thread/fork`, `thread/rollback`, or `turn/interrupt` request and the CLI remains the only rollout writer.
26. Refresh a selected thread whose initial detail briefly reports local running while another Codex client is the real writer; confirm the next runtime probe hands the thread to the external realtime path and immediately appends the selected active text tail.
27. With "When busy, send as" set to Queue, submit a mobile prompt while the external turn is still running and confirm it appears in the queue without starting a competing turn.
28. With "When busy, send as" set to Steer, submit a mobile prompt during an externally owned running turn and confirm it is persisted as one queued message without an optimistic sent bubble or competing `turn/start`. Repeat while runtime ownership changes from local to external, or another process wins the cross-process start claim, between submit and `turn/start`; confirm the rejected local attempt is removed and exactly one queued row remains with the original text, images, files, skills, and collaboration mode. Confirm managed uploads remain available to the durable queued row.
29. Confirm Stop cannot interrupt a direct CLI-owned turn. For a separately validated interruptible app-server-owned turn, confirm Stop uses the exact current active turn id and refreshes state without clearing visible output.
30. Confirm queued-message Steer, edit, delete, and reorder controls all remain disabled while an external writer owns the thread.
31. Queue a normal message with a managed image/file attachment behind a long-running turn, restart the safe service, and confirm the queued row and attachment remain available until that queued turn starts; advancing beyond the normal upload reaper TTL must not delete an attachment still referenced by durable queue state.
32. Stop and restart frontend polling through an account refresh/switch while a durable queued row exists; confirm the row is reloaded instead of leaving the queue panel empty.
33. During cold startup, mutate the durable queue after the first queue GET but before the notification stream reports ready; confirm the ready recovery snapshot shows the new queue state and does not resurrect an older revision.
34. Run two service processes against the same temporary CODEX_HOME, append/reorder/remove queue rows concurrently, and terminate one process after its durable append but before local scheduling; confirm no rows are lost or duplicated and the surviving process drains the committed row.
35. Submit an inverse-arrival queue row whose declared predecessor never reaches the server; confirm it waits for the bounded dependency grace period and then becomes eligible instead of remaining stuck forever.
36. Race a direct mobile turn start and a backend queue drain for the same idle thread from separate service processes; confirm exactly one start reaches app-server, the loser remains queued, and later queue reordering still controls execution order.
37. Disconnect the backend during an ambiguous queue append, then stop polling or switch account; confirm append/receipt retries use bounded backoff, stop immediately with the lifecycle reset, and release the optimistic row/upload lease.
38. Repeat the visible checks in dark theme.
...
- Opening or refreshing a CLI-owned thread is read-only and never creates a second writer, resumes the thread, starts a turn, forks, rolls back, or interrupts the CLI turn.
...
- Mobile submissions during external ownership are always append-only queued, including Steer mode, local-to-external ownership races, and cross-process start-claim conflicts; no external-steer marker or competing `turn/start` is issued. The complete normalized payload is preserved and managed uploads transfer to durable queue ownership.
...
- Managed attachments referenced by durable queue rows survive service restart and upload-reaper TTL. Once the queued row is accepted, attachment protection transfers to the active turn until terminal completion.
