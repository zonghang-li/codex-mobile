...
4. In Client B at `375x812` Light, read and scroll the conversation, then try to focus or edit the composer, add an attachment, change Goal/plan/model/reasoning/speed settings, submit Queue and plain-text Steer with the button and configured send shortcut, confirm Stop is exposed when the external writer is confirmed interruptible, edit a prior user prompt (rollback), undo/redo prior file changes, answer any pending server request, and edit, delete, drag, or reorder the queued row. Confirm queued-row Steer remains available for text-only queued messages.
...
- The thread header branch dropdown uses the confirmed external runtime cwd reported by live-state/thread-read, not the thread metadata cwd captured when the conversation was created. A thread created from `main` but currently running commands in a linked worktree shows that worktree's current branch, and falls back to the metadata cwd only when the runtime cwd is unavailable or no longer a readable Git worktree.
- Client B exposes Stop for a confirmed interruptible external active turn and sends `POST /codex-api/thread-runtime-interrupt` with the current thread and active turn id; it does not send local app-server `turn/interrupt` for that external writer. If runtime ownership is unknown or the writer is not confirmed interruptible, Stop remains hidden/disabled.
- If a refresh or startup race leaves Client B with a stale local running lease, a local `turn/interrupt` `thread not found` response triggers one targeted detail/runtime reconciliation. When the refreshed detail identifies an external active turn, Client B retries through `POST /codex-api/thread-runtime-interrupt` with that refreshed turn id instead of surfacing the stale local error.
- Client B's composer text, dictation-as-text input, and submit path remain usable for append-only Queue/Steer requests while external ownership is active; attachment, Goal, configuration, model, reasoning, and speed controls remain disabled.
- Queued edit, delete, drag, drop, and reorder controls are disabled in Client B, with no queue mutation; queued-row Steer remains enabled only for text-only queued messages and is disabled for queued rows that still carry images, skills, or file attachments.
...
- Client B does not drain persisted Queue-mode follow-ups while Client A still owns the external turn, including after Client B startup/recovery; plain-text queued-row Steer uses the explicit external steer path, and inconclusive Linux ownership checks keep the queue blocked and retry later.
...
- After the external turn reaches a terminal state, Client B removes the sidebar spinner, refreshes the completed conversation, keeps the normal idle send control, re-enables queue/composer interactions, and sends the follow-up normally.
...
4. Authenticate without placing the password in argv or the document, then query the runtime endpoint with an encoded thread ID. The synthetic Host header prevents the trusted-localhost shortcut, so this checks the authenticated path.
   ...
   jq -e '
     .state == "running"
     and .interruptible == true
     and .source == "external-session-writer"
     and (.turnId | type == "string" and length > 0)
     and ((keys - ["cwd"]) | sort == ["interruptible", "source", "state", "turnId"])
   ' "$EVIDENCE_DIR/running.json" || exit 1
   ```

   Expect the runtime probe to include `{"state":"running","turnId":"<turn-id>","interruptible":true,"source":"external-session-writer"}` and it may include `cwd` when the external writer worktree is known. The selected-thread detail layer should persist composer submissions to Queue, keep queued Steer controls disabled while ownership is external, expose Stop only for this interruptible app-server writer, and route Stop through `/codex-api/thread-runtime-interrupt` instead of local app-server `turn/interrupt`. A direct CLI writer must instead report `interruptible:false` and expose no Stop action.
