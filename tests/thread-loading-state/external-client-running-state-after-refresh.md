...
4. Authenticate without placing the password in argv or the document, then query the runtime endpoint with an encoded thread ID. The synthetic Host header prevents the trusted-localhost shortcut, so this checks the authenticated path.
   ...
   Expect the runtime probe to include `{"state":"running","turnId":"<turn-id>","interruptible":true,"source":"external-session-writer"}` and it may include `cwd` when the external writer worktree is known. The selected-thread detail layer should keep the mobile composer usable for text-only Queue/Steer, expose Stop for that external active turn id, and route Stop through `/codex-api/thread-runtime-interrupt` instead of local app-server `turn/interrupt`.
