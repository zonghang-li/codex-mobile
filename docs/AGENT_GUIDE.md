...
- `src/safe/ntfyTitle.ts`: normalized, bounded result-and-conversation notification-title contract with unnamed-thread fallback.
...
- `src/server/externalThreadRuntime.ts`: hardened same-user Codex CLI/app-server and canonical rollout-writer discovery.
...
11. Preserve notification behavior: threshold exactly `600_000` ms; result-specific title prefixes followed by a normalized conversation name; cached rename, then app-server `name`, then app-server `title` precedence; no prompt/preview-derived names; 80-code-point name cap and unnamed ID-suffix fallback; deterministic first non-empty assistant sentence capped at 180 characters; no extra AI call; five-second request timeout; three immediate attempts per pending record per drain; active/pending/sent collections capped at 256. Continue accepting legacy pending records containing the old fixed titles.
...
13. Accept external rollout writers only from current-UID Codex CLI or app-server processes with stable process/descriptor evidence. Canonicalize each regular `.jsonl` rollout and require it to remain under the canonical current-user Codex sessions root; exclude the safe service's own app-server process tree. Direct CLI writers are non-interruptible; only a revalidated app-server writer may be signalled.
14. Keep external monitoring serialized and bounded: scan tracked cursors before discovery at the 15-second production cadence, parse only complete newly appended JSONL records through bounded incremental cursors, and cap retained cursors. Evict a missing-writer cursor only after 24 hours without file growth or renewed writer evidence and only after a complete discovery scan confirms absence; capped scans and candidate canonicalization/stat failures retain missing cursors fail-closed. Successful validation may still conclusively reject unsafe paths, file types, or identities. Eviction never synthesizes a terminal lifecycle. Bound each Linux discovery cycle to 16,384 numeric processes, 4,096 descriptors per eligible Codex process, 8,192 yielded snapshots, 256 unique rollout writers, and 5,000 ms wall time.
...
| ntfy config/state/title | `src/safe/ntfyConfig.test.ts`, `src/safe/ntfyTitle.test.ts`, `src/safe/ntfyState.test.ts`, ownership/mode/symlink/atomic-write cases, title normalization/fallback, and legacy pending-title compatibility |
| ntfy qualification/delivery | `src/server/ntfyCompletionNotifier.test.ts`, including 599,999/600,000 ms, all terminal statuses, conversation-title precedence and fallback, real default headers, restart, retry, redaction, and direct-notification/external-rollout deduplication through one durable key and sequence ID |
...
`doctor` is a static packaged-invariant check, not proof that an optional topic is configured or that a phone received an alert. A qualifying notification requires a turn duration of at least 10 minutes (`600_000` ms). Completed, failed, and interrupted turns use `Codex 任务完成：<会话名称>`, `Codex 任务失败：<会话名称>`, and `Codex 任务已中断：<会话名称>`. Resolve the name from the cached desktop/mobile rename, app-server `name`, and app-server `title`, in that order; never use prompt or preview text. Normalize control/whitespace characters, cap the name at 80 Unicode code points, and fall back to `未命名会话（<thread ID last 8 characters>）`. The body is the deterministic first non-empty final-assistant sentence or a fixed fallback. The notifier retains only bounded timing/outbox/deduplication state under the safe home and never conversation history or the publish URL.

With ntfy enabled, `codex-mobile-safe` observes qualifying turns from the mobile UI, Codex Desktop, Codex CLI, and other same-user Codex clients that write authoritative rollouts. Detection is server-side and continues with no browser connection. The external monitor uses a serialized 15-second scan cadence and authoritative rollout timestamps, so a turn already running when the service restarts remains eligible; turns already terminal before restart are baseline history and are not replayed. A missing terminal lifecycle is never inferred from a Codex process or rollout-writer descriptor disappearing. A cursor whose writer is missing remains append-checked and is evicted only after 24 hours with neither file growth nor renewed writer evidence and a complete scan confirming absence. A capped or deadline-limited partial scan never expires an omitted cursor. Linux discovery is capped per cycle at 16,384 numeric processes, 4,096 descriptors per eligible Codex process, 8,192 yielded snapshots, 256 unique rollout writers, and 5 seconds.
